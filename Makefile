# This Makefile is for the OpenGL::Script extension to perl.
#
...
#	ABSTRACT => q[OpenGL scripting]
#	AUTHOR => q[Danil Pismenny <dapi@mail.ru>]
#	NAME => q[OpenGL::Script]
#	PREREQ_PM => { Time::HiRes=>q[1.2], OpenGL=>q[0.54], Error=>q[0.15] }
#	VERSION_FROM => q[./Script.pm]
#	clean => { FILES=>q[*.bz2*] }
...
# Where to put things:
INST_LIBDIR      = $(INST_LIB)/OpenGL
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/OpenGL
...
clean ::
	-rm -rf *.bz2* ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all perlmain.c mon.out core core.*perl.*.? *perl.core so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def $(BASEEXT).exp
